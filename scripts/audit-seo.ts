...
const auditLanguagePatterns = [
  /will not invent/i,
  /fake roster/i,
  /fake database/i,
  /unverified competitor/i,
  /rival wikis?/i,
  /fact pack/i,
  /SEO audit/i,
  /source confidence/i,
  /intentionally excluded/i,
  /speculation never/i,
];

function scanAuditLanguage(page: HomePageDefinition | SeoPageDefinition) {
  const text = pagePlainText(page);
  for (const pattern of auditLanguagePatterns) {
    if (pattern.test(text)) {
      errors.push(`${page.slug || "home"}: frontend audit-language phrase matched /${pattern.source}/i`);
    }
  }
}

scanAuditLanguage(homePage);
enabledCorePages.forEach(scanAuditLanguage);
