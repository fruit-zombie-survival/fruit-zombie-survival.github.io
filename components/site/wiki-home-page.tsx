    ...
    <>
      ...
      <main>
        <section className="hero-surface wiki-hero border-b border-border">
          <div className="site-container grid items-center gap-8 lg:grid-cols-[1.15fr_.85fr]">
            <div>
              ...
              <div className="wiki-hero-actions">
                ...
                {homePage.hero.secondaryInternalLink ? (
                  <Link href={routePath(homePage.hero.secondaryInternalLink.slug)} className="button-secondary">
                    <BookOpen size={18} />{homePage.hero.secondaryInternalLink.label}
                  </Link>
                ) : null}
            ...
            <aside className="overflow-hidden rounded-theme border border-border bg-card/60 shadow-theme" aria-label="Fruit Zombie Survival wiki cover">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img
                src={assetPath(siteConfig.assets.cover)}
                alt={`${siteConfig.game.name} wiki cover art`}
            ...
            sidebar={(
              <>
                ...
                <section className="wiki-site-info" aria-label="Site info">
                  ...
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={assetPath(siteConfig.assets.cover)} alt={`${siteConfig.game.name} wiki cover art`} />
                  <dl>
                    {facts.map(([term, value]) => (
                      <div key={term}>
                        <dt>{term}</dt>
